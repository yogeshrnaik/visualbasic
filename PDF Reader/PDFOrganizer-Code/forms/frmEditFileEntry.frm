...
Begin VB.Form frmEditFileEntry 
   ...
   Begin VB.ComboBox cbRecCount 
      Height          =   315
      ItemData        =   "frmEditFileEntry.frx":0000
      Left            =   8640
      List            =   "frmEditFileEntry.frx":0002
   ...
   Begin VB.CommandButton btnOpenPDF 
      ...
      Height          =   855
      Left            =   1440
      Picture         =   "frmEditFileEntry.frx":0004
      Style           =   1  'Graphical
      TabIndex        =   13
      Top             =   6720
      Width           =   1095
...
End Sub
'#################################################################################
Public Sub init_form(ByRef p_searchResults As Collection, sFilename As String, _
                      Optional p_curr_index As Integer = -1)
  Set m_searchResults = p_searchResults
  If (p_curr_index <> -1) Then
    m_curr_index = p_curr_index
     'get the PDFFileDetails object from the collection based on the sFilename
    Set m_objPDFDetails = m_searchResults.Item(m_curr_index)
  ...
  setFileStatus
  '-------------------------------------------------------------------------
  If (Me.cbRecCount.ListCount = 0) Or (Me.cbRecCount.ListCount <> m_searchResults.Count) Then
    populateRecordCountCombo
  End If
  '-------------------------------------------------------------------------
  Me.Show
  Me.SetFocus
...
End Sub
'#################################################################################
Private Sub populateRecordCountCombo()
  Me.cbRecCount.Clear
  Dim i As Integer
  For i = 1 To m_searchResults.Count
    Me.cbRecCount.AddItem (i)
  Next
  'Me.cbRecCount.ListIndex = 0
End Sub
'#################################################################################
'check whether PDF is present in filepath
Private Sub setFileStatus()
  If (GeneralMod.doesFileExist(m_objPDFDetails.m_filepath, m_objPDFDetails.m_filename)) Then
    Me.btnOpenPDF.Enabled = True
    Me.btnOpenPDF.ToolTipText = "File present in the specified folder."
  ...
  Else
    ...
    Me.btnOpenPDF.ToolTipText = "File not present in the specified folder."
...
End Function
'#################################################################################
Private Sub cbRecCount_Click()
  init_form m_searchResults, "", Me.cbRecCount.ListIndex + 1
End Sub
'#################################################################################
