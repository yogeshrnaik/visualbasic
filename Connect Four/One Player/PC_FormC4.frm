...
Private Sub DropBttn_Click(index As Integer)
    If player = "Y" Then
        If currInCol(index) >= 0 Then
            ...
            player = "R" 'change turn
            updatePosition (index)
            checkGameStatus
        End If
    End If
    'computer will play now at random position
    If player = "R" Then
        Dim playInCol As Integer
        Dim done As Boolean
        WhoseTurn.Refresh
        Dim xyz As Double
        For xyz = 0 To 5000000
        Next
        
        'check if randomly generated move is legal or not
        done = False
        Randomize
        While (Not done)
            playInCol = (Rnd * 100) Mod 6
            If currInCol(playInCol) >= 0 Then
                done = True
            End If
        Wend
        CirclePos(currInCol(playInCol)).FillColor = &HFF&     'red
        gamePos((currInCol(playInCol) - playInCol) / 6, playInCol) = "R"
        WhoseTurn.Caption = "    Yellow     Player's        Turn"
        WhoseTurn.BackColor = &HFFFF&   'yellow
        player = "Y"
        updatePosition (playInCol)
        checkGameStatus
    End If
End Sub
Private Sub updatePosition(index As Integer)
    'update position
